...
BASELINE_FEATURES = ['Volume', 'Open', 'High', 'Low', 'Close', 'Scaled_sentiment']
TARGET_FEATURE_INDEX = BASELINE_FEATURES.index('Close') 
...
def data_processor(data, target_feature_index):
    ...
    return dataloader_train, dataloader_test, scaler_X, scaler_y
...
def train_model(dataloader_train, symbol, num_csvs, d_input, run_type):
    ...
    model_dir = "model_saved_close_only" 
    ...
    for idx_epoch in range(epochs):
        ...
        with tqdm(total=len(dataloader_train.dataset), desc=f"[Epoch {idx_epoch+1:3d}/{epochs}] {symbol}") as pbar:
            for idx_batch, (x, y) in enumerate(dataloader_train):
                if x.size(0) != dataloader_train.batch_size or y.size(0) != dataloader_train.batch_size:
                    continue 
                optimizer.zero_grad()
                y_pred = model(x)
...
def eval_model(model, dataloader_test, symbol, num_csvs, scaler_y, output_length, run_type, summary_csv_path):
    ...
    if raw_true_clean.shape[0] > 0:
        ...
        try:
            with open(summary_csv_path, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([symbol, mae_raw, mse_raw, r2_raw])
            print(f"Successfully appended metrics to {summary_csv_path}")
        except Exception as e_csv:
            print(f"ERROR: Could not write to summary file {summary_csv_path}: {e_csv}")
        ...
        try:
            with open(summary_csv_path, mode='a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([symbol, np.nan, np.nan, np.nan])
            print(f"Successfully appended NaN metrics to {summary_csv_path} for {symbol}")
        except Exception as e_csv:
            print(f"ERROR: Could not write to summary file {summary_csv_path}: {e_csv}")
...
def run_baseline_predict(csv_data, symbol, num_csvs, pred_flag, pred_names, summary_csv_path):
    run_type = "BASELINE_CLOSE" 
    selected_columns = BASELINE_FEATURES

    if csv_data is None: print(f"Error: No data for {symbol}."); return
    csv_data.columns = csv_data.columns.str.capitalize()
    required_cols_present = all(col.capitalize() in csv_data.columns for col in selected_columns)
    if not required_cols_present:
        selected_columns = ['Volume', 'Open', 'High', 'Low', 'Close', 'Scaled_sentiment']
        if not all(col in csv_data.columns for col in selected_columns):
             print(f"Error: Missing BASELINE columns for {symbol}. Needed: {selected_columns}"); return
    try:
    ...
    dataloader_train, dataloader_test, _, scaler_y = processed_data
    ...
    if model is not None and dataloader_test is not None and len(dataloader_test.dataset) > 0:
         print(f"\n--- Evaluating BASELINE CLOSE for {symbol} ---")
         
         eval_model(model, dataloader_test, symbol, num_csvs, scaler_y, output_length=3, run_type=run_type, summary_csv_path=summary_csv_path)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in ['5', '25', '50']:
        print("\nUsage: python runbaseline_close_only.py [5|25|50]"); sys.exit(1)
    ...
    summary_csv_path = f"summary_results_{run_type}_{num_stocks}stocks.csv"
    print(f"Logging summary metrics to: {summary_csv_path}")
    
    if not os.path.isfile(summary_csv_path):
        try:
            with open(summary_csv_path, mode='w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['Stock', 'MAE', 'MSE', 'R2'])
        except Exception as e_csv:
            print(f"ERROR: Could not create summary file header {summary_csv_path}: {e_csv}. Exiting.")
            sys.exit(1)


    data_folder = "processed_data_baseline" 
    ...
    for name in names_to_process:
        ...
        essential_cols = ['Date', 'Volume', 'Open', 'High', 'Low', 'Close', 'Scaled_sentiment']
        if not all(col.capitalize() in csv_data.columns.str.capitalize() for col in essential_cols):
             if not all(col in csv_data.columns for col in ['Volume', 'Open', 'High', 'Low', 'Close', 'Scaled_sentiment']):
                 print(f"Warning: Missing essential columns in {data_path}. Skipping."); continue
        try:
        ...
        run_baseline_predict(csv_data, symbol_name, num_stocks, True, pred_names, summary_csv_path)
