...
RUN_TYPE_NAME = "BASELINE_XGB_CLOSE"
SELECTED_COLUMNS = ['Date', 'Volume', 'Open', 'High', 'Low', 'Close', 'Scaled_sentiment']
    ...
    try:
        ...
        if matching_files:
            ...
            try:
                ...
                print(f"Warning: File {matching_files[0]} is empty. Skipping.")
                return None
            except Exception as e_inner:
                print(f"An error occurred reading {matching_files[0]}: {e_inner}")
                return None
        else:
...
def run_xgb_experiment(csv_data, symbol, num_csvs, pred_flag, pred_names):
    ...
    try:
        ...
        print(f"Error: Target column '{TARGET_FEATURE_NAME}' not found in data or SELECTED_COLUMNS. Skipping.")
        return None
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in ['5', '25', '50']:
        print(f"\nError: You must specify the number of stocks to run.")
        print(f"Usage: python {sys.argv[0]} [5|25|50]")
        sys.exit(1)
